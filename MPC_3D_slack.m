function [xtot, utot, cost] = MPC_3D_slack(init,params,pred,CWH)
...
% the params structure. Constraints are implemented as soft using 8 slack
% variables, one for each face on the cone. 
...
Q = 10^3.*eye(6); R = 10^6.*eye(11); R(1:3,1:3) = 100.*eye(3);
...
while norm(x0vec(1:3))>=(rp+rs)
    counter = counter + 1;
    disp(['Running optimization ',num2str(counter),', distance from origin is ',num2str(norm(x0vec(1:3)))])
    cvx_begin quiet
    variables X(n,N+1) U(m,N)
    X(:,2:N+1) == sysD.a*X(:,1:N) + sysD.b*U;
    X(:,1) == x0vec;
    max(C*X(:,1:Nc)+D*U(:,1:Nc)) <= 0;
    max(U(1,:).^2 + U(2,:).^2 + U(3,:).^2) <= Umax^2;
